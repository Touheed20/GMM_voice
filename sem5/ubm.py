...
if __name__ == "__main__":
    ...
    UBM_MeanVec = np.zeros(shape=(NoM,60))
    UBM_VarVec = np.ones(shape=(NoM,60))
    UBM_wVec = np.ones(shape=(NoM))

    
    X =  np.genfromtxt('ubm0_d.csv')
...
##            print wVec
    ...
    UBM_MeanVec = np.asarray(UBM_MeanVec)
    UBM_VarVec = np.asarray(UBM_VarVec)
    UBM_wVec = np.asarray(UBM_wVec)
    UBM_MeanVec.tofile('UBMMean.txt',sep=',',format='%10.5f')
    UBM_VarVec.tofile('UBMVar.txt',sep=',',format='%10.5f')
    UBM_wVec.tofile('UBMWgt.txt',sep=',',format='%10.5f')
